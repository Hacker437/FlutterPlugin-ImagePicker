...
public class ImagePickerDelegate
  ...
  public void chooseIconFromGallery(MethodCall methodCall, MethodChannel.Result result) {
    if (!setPendingMethodCallAndResult(methodCall, result)) {
      finishWithAlreadyActiveError(result);
      return;
    }

    launchPickIconFromGalleryIntent();
  }
  ...
  private void launchPickIconFromGalleryIntent() {
    ...
    Intent chooserIntent = Intent.createChooser(galleryIntent, "Select icon");
    chooserIntent.putExtra(Intent.EXTRA_INITIAL_INTENTS, new Intent[] { fileIntent });
    activity.startActivityForResult(chooserIntent, REQUEST_CODE_CHOOSE_IMAGE_FROM_GALLERY);
  }
